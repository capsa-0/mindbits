...
from archs import FeedForward as NeuralNetwork

m_rate = Config.MUTATION_RATE
...
class Individual:
    ...
    def __init__(self, parent=None, nn = 0, color = 0):

        if color:
            self.color = color
        else:
            self.color = self.generate_color(parent)
        ...
        if parent:
            self.nn = copy.deepcopy(parent.nn)
            self.nn.mutate(mutation_rate=m_rate)
        elif nn:
            self.nn = nn
        else:
            self.nn = NeuralNetwork(Config.INPUT_SIZE, Config.NN_ARCHITECTURE)
    ...
    def decide_movement(self, terrain, population_map, food_map):
        ...
        output = self.nn.forward(one_hot_input)
    ...
    def start_cooldown(self):
        ...
        self.fitness += 1  # Aumentar fitness al reproducirse
    ...
    def generate_color(self, parent=0):
        if parent:
            np_color = tuple(np.clip(parent.original_color + np.random.randint(-20,20,3), 0, 255))
            color_python = tuple(map(int, np_color))
            return color_python
        
        np_color = (random.randint(0,255), random.randint(0,255), random.randint(0,255))
        color_python = tuple(map(int, np_color))
        return color_python
   ...
   # def reproduce(self):
    #    self.process_mating()
     #   for ind in self.population:
      ...
      #          self.next_gen.append(Individual(ind))
    ...
    def get_next_gen(self, elitism=Config.ELITISM):
        # Ordenar la población por fitness
        sorted_population = sorted(self.population, key=lambda x: (x.fitness), reverse=True)

        fitness = [ind.fitness for ind in sorted_population]

        conteo = Counter(fitness)
        plt.bar(list(conteo.keys()), list(conteo.values()))
        plt.title(f'GEN {self.generation_count}')
        plt.xlabel("Fitness", labelpad=10)
        plt.ylabel("Frecuencia absoluta", labelpad=10)
        plt.savefig("out/fitness_bar.png")
        plt.close()

        n_elite = int(len(sorted_population) * elitism)
        elite = sorted_population[:n_elite]
        for ind in elite:
            ind.elite = 1

        n = int(Config.P_SURVIVORS*Config.POPULATION_SIZE)
        repetitions = int(1/Config.P_SURVIVORS)
        best_n = sorted_population[:n] * repetitions
        next_gen = elite + [Individual(parent=ind) for ind in best_n[:Config.POPULATION_SIZE-n_elite]]
    ...
    def run_generation(self):
        ...
        while  (self.mating_count*2<Config.POPULATION_SIZE or self.simulation_count <= Config.MIN_ITER) and self.running:
        ...
        self.save_gen_data(self.mating_count*2, self.simulation_count)

        self.population = self.get_next_gen()
        


    def run_blind_generation(self):
        ...
        while (self.mating_count*2<Config.POPULATION_SIZE or self.simulation_count <= Config.MIN_ITER) and self.running:
            ...
            if self.running:
                ...
                print(f'GEN {self.generation_count} | SIM {self.simulation_count} | COUNT {self.mating_count*2}/{Config.POPULATION_SIZE}')

                self.simulation_count += 1

        self.save_gen_data(self.mating_count*2, self.simulation_count-1)
        ...
        if blind:
            for n in range(n):
                ...
                if self.generation_count % 5 == 0:
                    pop_management.guardarPoblacion(self.population, "out/pob.json", "nn", self.generation_count, self.repro_rate)
    ...
    def display_info(self):
        ...
        gen_text = font.render(f"GEN {self.generation_count} | sim {self.simulation_count} | cruces {self.mating_count} | paso {self.step}", True, (255, 255, 255)) 
        self.screen.blit(gen_text, (Config.INFO_X, 10))


        # Configuración mostrada
        down_text=f'Tamaño pob - {Config.POPULATION_SIZE} | Ratio de mutación - {m_rate} | Alfa de mutación - {Config.MUTATION_SCALE} | Radio de visión - {Config.VISION_RADIUS} | Tamaño memoria - {Config.MEMORY_LENGTH} | Arquitectura - {[Config.INPUT_SIZE] + Config.NN_ARCHITECTURE + [9]} | Parámetros  - {Config.N_PARAMETERS}' 
    ...
    def update_mutation_rate(self):
        ...
        if m_rate >= 0.0001:
            m_rate = round(Config.MUTATION_RATE * Config.MUTATION_RATE_DECAY**self.generation_count, 3)


def create_sim(pop_file):
    pop, n_gen, repro_rate= pop_management.cargarPoblacion(pop_file, Individual, NeuralNetwork, Config.INPUT_SIZE, Config.NN_ARCHITECTURE)
    sim = Simulation(pop, n_gen, repro_rate)
    return sim

if __name__ == "__main__":
    sim = Simulation()
    #sim = create_sim("out/pob.json")
    sim.run(1, Config.BLIND_GENS)
    sim.run()