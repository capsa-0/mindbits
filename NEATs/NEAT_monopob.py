...
class Simulation:
    def __init__(self, population = [Individual() for _ in range(Config.POPULATION_SIZE)], n_gen=0, n_genome=0, repro_rate=[]):
    ...
    def run_generation(self, n_sims=10):
        ...
        while self.simulation_count <= n_sims:
            ...
            self.food = MapManager.spawn_food(self.terrain)
            self.total_food += np.count_nonzero(self.food == 2)
            self.step = 0
            ...
            if self.running:
                ...
                while any(ind.alive for ind in self.population) and self.running:
                    ...
                    if 1<self.step and np.array_equal(population_map, self.get_population_map()):
                        break
        ...
        pob_fitness = sum(fit)/self.total_food
    ...
    def run_blind_generation(self, n_sims=10):
        ...
        print(f'GEN {self.generation_count} | genome {self.n_genome} | fitness {pob_fitness:.3f}')
        return pob_fitness
    ...
    def display_info(self):
        ...
        gen_text = font.render(f"GEN {self.generation_count} | genome {self.n_genome} | sim {self.simulation_count} | paso {self.step}", True, (255, 255, 255)) 
...
def evaluate_genomes1(genomes, config):
    ...
    for genome_id, genome in genomes:
        population = [Individual(genome,species_id=species[genome_id]) for i in range(Config.POPULATION_SIZE)]
        sim = Simulation(population, n_gen=counter, n_genome=j)

        pop_fitness = sim.run_blind_generation(n_sims=Config.MIN_ITER)

        genome.fitness = pop_fitness

        fitness.append(pop_fitness)
...
def evaluate_genomes2(genomes, config):
    ...
    for genome_id, genome in genomes:
        ...
        sim = Simulation(population, n_gen=counter, n_genome=j)
        sim.init_pygame()
        pop_fitness = sim.run_generation(n_sims=Config.MIN_ITER)
